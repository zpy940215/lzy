...
function($, common, jqueryui, ztree, Handlebars, HandlebarExt, diyUpload, moment, stay, viewer, main,base64, UE) {
	...
	var editList = {
		...
		saveView: function() {
			...
			var score = $("#score").val();
			if(score != '') {
				if(!common.validate.isNumber('#score', '请输入大于0且小于5的数字！')) {
					checkResult = false;
				}
				if(score<0 || score>5) {
					$("#score").siblings(".js_errortip").html('请输入大于0且小于5的数字！');
					checkResult = false;
				}
			}
					...
					if (resultMap.code == "success") {
						...
						common.base.popUp('', {
							...
							backFn: function(result) {
								if (result) {
									//返回列表
									window.location.href = '${base}/tourdata/food/list.html';
								}
							}
						});
		...
		uploadpic: function() {
			...
			$('.js_picupload').click(function() {
				...
				// 素材库选择
				$(".materialbox ul.materiallist li").click(function() {
			...
			$(document).on("click",'.js_Cancel',function(){
				...
				if(picobj.hasClass('old')) {
					common.base.popUp('', {
						...
						backFn: function(result) {
							if (result) {
								var picId = picobj.parent('.picinsert').find('img').attr('picId');
								$.ajax({
									type: "post",
									...
									success: function(resultMap) {
										if (resultMap.code == "success") {
											picobj.parent('.picinsert').remove();
										} else {
											if (resultMap && resultMap.description) {
												alert(resultMap.description);
											} else if (resultMap && typeof(resultMap) == 'string') {
