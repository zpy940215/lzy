...
function($, common, Handlebars, jqueryui, HandlebarExt, ztree) {
	...
	var shop = {
		...
		shopList: function() {
			if (shop.isAjaxing) return false;
			shop.isAjaxing = true;
			// alert($("#js_status").find("option:checked").attr("value"));
			$.ajax({
				url: '${base}/view!queryListPage.ajax',
				data: {
					"pageObject.page": shop.pageCur,
					"pageObject.pagesize": shop.pageSize,
					...
					"viewVo.type": "shop",
					"viewVo.smallType": shop.viewType
				},
				type: 'post',
				dataType: "json",
				beforeSend: function() {
				...
				success: function(resultData) {
					...
					if (resultData.code == "success") {
						...
						//分页
						shop.pageTotal = resultData.data.pagetotal;
						shop.total = resultData.data.total;
						shop.dataList = resultData.data.dataList;
						common.base.createPage('.pageDiv', {
							pageCur: shop.pageCur,
							pageTotal: shop.pageTotal,
							total: shop.total,
							backFn: function(p) {
								shop.pageCur = p;
								shop.shopList();
						...
						shop.shopEdit(); //初始化编辑事件
						shop.shopDelete(); //初始化删除事件
						shop.checkbox(); //初始化全选事件
						shop.sortable(); //初始化排序事件
						shop.sortchange();
			...
			function() {
				...
				common.base.popUp('', {
					...
					backFn: function(result) {
						if (result) {
							var status = "close";
							shop.shopDoDelete(Id, viewId, status);
						}
		...
		queryByshopId: function(projectId) {
			var returnData;
			$.ajax({
		...
		sortchange: function() {
			$('.jspos').blur(function() {
				...
				if (inputPos != pos) {
					shop.shopDoEdit(id, viewId, inputPos);
				}
			...
			$(function() {
				$("#sortable").sortable({ //这里是talbe tbody，绑定 了sortable
					...
					stop: function(event, ui) {
						...
						shop.shopDoEdit(data['id'], data['viewId'], pos);
					}
				}).disableSelection();
			})
		},

		shopDoEdit: function(Id, viewid, pos, statuval) {
			//          if(shop.isAjaxing)return false;
		...
		shopDoDelete: function(Id, viewId, status) {
			if (!Id) {
				shop.isAjaxing = false;
				return false;
			}
	...
	//景区
	var tourShopListInit = function(rootAreaid) {
		shop.rootAreaid = rootAreaid;
		//查询区域
		shop.areaList();
		//搜索
		...
		function() {
			...
			} else if (window.confirm("是否确定删除选中数据？")) {
				common.base.popUp('', {
					type: 'choice',
					tipTitle: '温馨提示',
					//标题
						...
						if (result) {
							...
							rowchecked.each(function() {
								...
								if (shopIds != null && shopIds != '') {
									shopIds = shopIds + "," + thisid;
								} else {
									shopIds = thisid;
								}
								if (shopViewIds != null && shopViewIds != '') {
									shopViewIds = shopViewIds + "," + thisViewid;
								} else {
									shopViewIds = thisViewid;
								}
								count++;
								if (count == rowchecked.length) {
									var status = "close";
									shop.shopDoDelete(shopIds, shopViewIds, status);
								}
							});
		...
		function(e) {
			...
			if (rowchecked.length == 1) {
				...
				if (($('tr').index(_current) - 2) >= 0) {
					...
					var thisPos = (shop.pageTotal - shop.pageCur) * shop.pageSize + (shop.pageSize - currentLine) + 1;
					//					var thisPos = _current.find(
					//					".id").attr('pos');
					//上一级
					var getPosval = (shop.pageTotal - shop.pageCur) * shop.pageSize + (shop.pageSize - currentLine + 1) + 1;
					...
					shop.shopDoEdit(thisid, thisviewId, getPosval, "0");

					shop.shopDoEdit(preId, previewId, thisPos, "1")
					//					var v =2;
					//					for(var i=0;i<2;i++){
		...
		function(e) {
			...
			if (rowchecked.length == 1) {
				...
				if (trLength > (currentIndex + 1)) {
					...
					var nextPos = (shop.pageTotal - shop.pageCur) * shop.pageSize + (shop.pageSize - currentLine - 1) + 1;
					var nextViewId = _current.next('tr').find('.id').attr('viewid');
					var thisviewId = _current.find(".id").attr('viewid');
					var thisPos = l = (shop.pageTotal - shop.pageCur) * shop.pageSize + (shop.pageSize - currentLine) + 1;
					//                    _current.insertBefore(_current.prev());
					var thisid = _current.find(".id").val();

					shop.shopDoEdit(thisid, thisviewId, nextPos, "0");
					shop.shopDoEdit(nextId, nextViewId, thisPos, "1");
					...
					backFn: function(result) {
						if (result) {
							...
							rowchecked.each(function() {
								...
								//								alert(sortNum)
								var thisViewid = $(this).parents('tr').find(".id").attr('viewid');
								shop.shopDoEdit(thisid, thisViewid, sortNum);
							})
						}
					}
				});
