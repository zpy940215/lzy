	...
	var scenic = {
		...
		areaList: function() {
			$.ajax({
				...
				type: 'POST',
				dataType: "json",
				url: '${base}/area!queryListAll.ajax',
				data: {
					"areaVo.code": scenic.rootAreaid
				},
				success: function(data) {
					...
					var zNodes = data.data.areaVoList;
					$.fn.zTree.init($("#areatreeDemo"), setting, zNodes);
					//树形结构加载成功之后，再查询列表
					scenic.scenicList();

					function zTreeOnClick(event, treeId, treeNode) {
						//								alert(treeNode.level+","+treeNode.id+","+treeNode.name);
						scenic.areaCode = treeNode.code;
						//								alert("长度："+scenic.areaCode.length);
						if (scenic.areaCode.length > 4) {
							//									栏目节点筛选
							scenic.scenicList();
						} else {
							scenic.areaCode = "";
