...
function($, common, jqueryui, ztree, Handlebars, HandlebarExt, diyUpload, moment, stay, timepicker, UE, viewer, main, base64) {
	...
	var editList = {
		...
		save: function() {
			...
			function() {
				...
				postData['prodVo.viewId'] = $("#selTicket").val();
				...
				//规格
				/*
				var prodPrice=$('#prodPrice_num').find('tbody');
				for(var i=0;i<prodPrice.length;i++){
					var personNum=prodPrice.eq(i).find('.personNum').val(),
					 childNum=prodPrice.eq(i).find('.childNum').val(),
					 oldPrice=prodPrice.eq(i).find('.oldPrice').val(),
					 price=prodPrice.eq(i).find('.price').val(),
					 totalNum=prodPrice.eq(i).find('.totalNum').val(),
					 freeNum=prodPrice.eq(i).find('.freeNum').val(),
					 desc=prodPrice.eq(i).find('.desc').val(),
					 prodPriceId=prodPrice.eq(i).find('.prodPriceId').val();
					
					 postData['prodPriceVoList['+i+'].param1Name'] = "最小预定量";
					 postData['prodPriceVoList['+i+'].param1Value'] = personNum;
					 postData['prodPriceVoList['+i+'].param2Name'] = "最大预定量";
					 postData['prodPriceVoList['+i+'].param2Value'] = childNum;
					 postData['prodPriceVoList['+i+'].price'] = oldPrice;
					 postData['prodPriceVoList['+i+'].discountPrice'] = price;
					 postData['prodPriceVoList['+i+'].totalNum'] = totalNum;
					 postData['prodPriceVoList['+i+'].freeNum'] = freeNum;
					 postData['prodPriceVoList['+i+'].param3Name'] = "门票类型";
					 postData['prodPriceVoList['+i+'].param3Value'] = desc;
					 postData['prodPriceVoList['+i+'].id'] = prodPriceId;
				...
				$.ajax({
					...
					success: function(resultMap) {
						editList.isAjaxing = false;
						if (resultMap.code == "success") {
							window.location.href = '${base}/prodnew/ticket/list.html';
							return;
						} else {
							if (resultMap && resultMap.description) {
								alert(resultMap.description);
	...
	var editListInit = function(description, content, costExplain, bookedExplain, othersExplain, param,viewId) {
		...
		$("#selTicket").find("option[value="+viewId+"]").attr("selected",true);
