...
function($, common, jqueryui, ztree, Handlebars, HandlebarExt, diyUpload, UE,viewer, main, base64) {
	...
	var editList = {
					...
					$(".picset").each(function() {
						...
						$('#' + thisid).diyUpload({
							...
							success: function(data, thisid) {
								...
								hotelPicArray.splice(index, 1, data.picUrl);
							},
							error: function(err) {
								console.info(err);
							}
		...
		save: function() {
			$("#producteditsubmit").click(function() {
				...
				postData['prodVo.viewId'] = $("#selHotel").val();
				...
				$.ajax({
					...
					success: function(resultMap) {
						editList.isAjaxing = false;
						if (resultMap.code == "success") {
							window.location.href = '${base}/prodnew/hotel/list.html';
							return;
						} else {
							if (resultMap && resultMap.description) {
								alert(resultMap.description);
	...
	var editListInit = function(description, content, costExplain, bookedExplain) {
