...
function($, common, jqueryui, timepicker,Handlebars, HandlebarExt) {
	var priceSet = {
		...
		changePirce:function(){
			...
			$('.js_price_changebtn').click(function() {
				if(!priceSet.isAjaxing){
					...
					datapost['prodPriceVo.param1Name'] = "最小购票量";
					datapost['prodPriceVo.param1Value'] = $('.param1Value_'+prodPriceId).val();
					datapost['prodPriceVo.param2Name'] = "最大购票量";
					datapost['prodPriceVo.param2Value'] = $('.param2Value_'+prodPriceId).val();
		...
		deletePrice:function(){
			...
			$('.js_price_deletebtn').unbind("click");
			$('.js_price_deletebtn').click(function() {
				var prodPriceId=$(this).attr("prodPriceId");
				common.base.popUp('', {
					type: 'choice',
					tipTitle: '温馨提示',
		...
		setPrice:function(){
			...
			$('.js_priceSet_set').click(function() {
				if(!priceSet.isAjaxing){
					...
					datapost['prodPriceVo.price'] = $('.js_price_menshi ').val();
					datapost['prodPriceVo.discountPrice'] = $('.js_price_xiaoshou').val();
					datapost['prodPriceVo.totalNum']=$('.js_price_totalNum').val();
					datapost['prodPriceVo.freeNum']=$('.js_price_freeNum').val();
					datapost['prodPriceVo.param1Name'] = "最小购票量";
					datapost['prodPriceVo.param1Value'] = $('.js_price_param1Value').val();
					datapost['prodPriceVo.param2Name'] = "最大购票量";
					datapost['prodPriceVo.param2Value'] = $('.js_price_param2Value').val();
					datapost['prodPriceVo.param3Name'] = "购票说明";
