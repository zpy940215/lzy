...
function($, common, jqueryui, Handlebars, HandlebarExt) {
	...
	var ticketList = {
		...
		getList: function() {
			...
			if ($("#selectChildSpotId").val() != '') {
				ticketList.linkSpots = $("#selectChildSpotId").val();
			} else if ($("#selectSpotId").val() != '') {
				ticketList.linkSpots = $("#selectSpotId").val();
			} else if ($("#selectSenicId").val() != '') {
				ticketList.linkSpots = $("#selectSenicId").val();
			} else {
			...
			$.ajax({
				...
				success: function(resultData) {
					...
					if (resultData.code == "success") {
						...
						ticketList.syncAgengProd();
						ticketList.sortchange();
					} else {
		...
		saveApp:function() {
			$('.js_appset_selectday').unbind("click");
		    $(".js_appset_selectday").click(function() {
			var configVal = $('.js_app_selectday').val();
			common.base.popUp('', {
				type: 'choice',
				tipTitle: '温馨提示',
				//标题
				...
				backFn: function(result) {
					if (result) {
						...
						ticketList.isAjaxing = true;
						$.ajax({
							url: '${base}/prod!saveProjectConfig.ajax',
							data: {
								"configVal":configVal
							},
							dataType: "json",
		...
		//同步票务云
		syncAgengProd:function(){
			$('.js_sync_agent').click(function() {
				$.ajax({
					url: '${base}/prod!agentSyncProd.ajax',
					data: {"outProdCode":$(".js_sync_agent").val()},
					dataType: "json",
					success: function(resultData) {
						if (resultData.code == "success") {
							common.base.popUp('', {
		...
		sortchange: function() {
			$('.jspos').blur(function() {
				...
				if (pos != value) {
					data['prodVo.id'] = id;
					data['prodVo.pos'] = value;
					$.ajax({
						async: false,
						cache: false,
						type: 'POST',
						dataType: "json",
						url: '${base}/prod!editPos.ajax',
						data: data,
