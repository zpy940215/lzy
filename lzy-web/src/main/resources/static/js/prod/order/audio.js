			...
			$('.js_detail_order').on('click',function(){
				...
				$.ajax({
					...
					success: function(result) {
						...
						}else if(result.data.orderVo.bizType == "audio") {
							var _templ = Handlebars.compile($("#order-detail-audio").html());
							$("#audio-detail-content").html(_templ(result.data.orderVo));
							common.base.popUp('.js_popUpAudioDetail');
						}
						order.editIdcard();
						order.applyRefund();
						order.refund();
