	...
	var article = {
		...
		articleList: function() {
			...
			$.ajax({
				...
				data: {
					"pageObject.page": article.pageCur,
					"pageObject.pagesize": article.pageSize,
					"articleVo.subject": keyword,
					"articleVo.siteId": article.siteId,
					"articleVo.categoryIds": article.categoryId,
					"articleVo.isRecommend": $("#isrecommend").find("option:checked").attr("value"),
					"articleVo.status": $("#ispush").find("option:checked").attr("value")
				},
				type: 'post',
				dataType: "json",
				beforeSend: function() {
		...
		articleSave: function(id, isRecomment) {
			...
			$.ajax({
				...
				data: {
					'articleVo.id': id,
					"articleVo.isRecommend": isRecomment,
					"articleVo.onlyrecomment": "Y"
				},
				beforeSend: function() {
					common.base.loading("fadeIn");
				},
...
//		审核不通过：noPass
		audit: function() {
			$('.info_row_audit').unbind('click');
			$('.info_row_audit').on('click', function() {
				var id = $(this).attr('dataId');
				var auditDesc = $(this).attr('auditDesc');
				var _templ = Handlebars.compile($("#articleexamine-save").html());
				$("#article-examine-content").html(_templ());
				common.base.popUp('.js_popUpuparticle');
				$('.js_popUpSubmit').on('click',function() {
					var reason = $('.js_reason').val();
					article.auditSave(id,"open",reason);
					$('.js_popUpuparticle').fadeOut();
				});
				$('.js_nopass').on('click',function() {
					var reason = $('.js_reason').val();
					article.auditSave(id,"noPass",reason);
					$('.js_popUpuparticle').fadeOut();
				});
			});
		},
		auditSave:function(id,status,reason) {
			if (article.isAjaxing) return false;
			article.isAjaxing = true;
			$.ajax({
				type: "post",
				...
				data: {
					'articleVo.id': id,
					"articleVo.status": status,
					"articleVo.auditDesc": reason
				},
				beforeSend: function() {
					common.base.loading("fadeIn");
				},
			...
			$(function() {
				$("#sortable ").sortable({ //这里是talbe tbody，绑定 了sortable
					...
					stop: function(event, ui) {
						...
						var articleVo = dataList[startIndex];
						var changeVo = dataList[endIndex];
						if (changeVo.pos == null || isNaN(changeVo.pos)) {
							changeVo.pos = '0';
						}
						if (parseInt(offsetIndex) < 0) {
							articleVo['pos'] = parseInt(changeVo.pos) + 1;
						} else {
							articleVo['pos'] = parseInt(changeVo.pos) - 1;
						}
						var data = {};
						data['articleVo.id'] = articleVo.id;
						data['articleVo.pos'] = articleVo.pos;
						data['articleVo.articleId'] = articleVo.articleId;
						$.ajax({
							async: false,
							cache: false,
							type: 'POST',
							dataType: "json",
							url: '${base}/article!editPos.ajax',
							data: data,
		...
		sortchange: function() {
			$('.jspos').blur(function() {
				...
				var data = {};
				if (pos != value) {
					data['articleVo.articleId'] = articleId;
					data['articleVo.pos'] = value;
					data['articleVo.id'] = vitulId;
					$.ajax({
						async: false,
						cache: false,
						type: 'POST',
...
//					}
					...
					article.categoryId = treeNode.newcategoryId;
					article.siteId = treeNode.currentSiteId;
					article.needAudit = treeNode.needAudit;
	...
	var articleListInit = function() {
		var categoryId = article.getQueryString("articleVo.categoryId");
		if (categoryId != '') {
