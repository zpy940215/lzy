...
function($, common, jqueryui, Handlebars, HandlebarExt, moment, stay) {
	...
	var order = {
		...
		orderList:function(){
			...
			data['payLogDayVo.startDate'] = $(".js_order_startDate").val()+" 00:00:00";
			data['payLogDayVo.endDate'] = $(".js_order_endDate").val()+" 23:59:59";
			
			$.ajax({
				url: '${base}/paylogday!queryPageList.ajax',
				data: data,
				type: 'post',
				dataType: "json",
				beforeSend: function() {
		...
		orderTotal:function(){
			...
			$.ajax({
				...
				success: function(resultData) {
					if (resultData.code == "success") {
						$(".js_totalPrice").html("有效收入：￥"+resultData.data.totalPrice);
		...
		exportExcel:function(){
			...
			$('.js_excel_btn').on('click',function(){
				...
				data['pageObject.pagesize'] = order.total;
				$.ajax({
					url: '${base}/paylogday!download.ajax',
					data: data,
					type: 'post',
					dataType: "json",
